...
import { aggregate, DEFAULT_COLUMNS, DISPLAY_METRICS, EMPTY_FILTERS, dateRange, filterSessions, formatMetric, metricValue, METRICS, sortRows, toCsv, type DisplayMetric, type Filters, type SessionRow, type SortKey } from "../shared/model";
...
const preferences = new Map<string, { filters: Filters; columns: DisplayMetric[]; sort: SortKey; direction: "asc" | "desc" }>();
const PAGE_SIZE = 40;
...
function UsageView({ theme, layout, host, navigation }: PluginSurfaceProps) {
  ...
  function remember(next: Partial<{ filters: Filters; columns: DisplayMetric[]; sort: SortKey; direction: "asc" | "desc" }>) { preferences.set(host.id, { filters, columns, sort, direction, ...next }); }
  function changeFilters(value: Filters) { setFilters(value); setPage(0); remember({ filters: value }); }
  function changeSort(key: SortKey) { const next = sort === key && direction === "desc" ? "asc" : "desc"; setSort(key); setDirection(next); setPage(0); remember({ sort: key, direction: next }); }
  function header(key: SortKey, title: string, width: number) {
    return <Pressable key={key} accessibilityRole="button" accessibilityLabel={`Sort by ${title}${sort === key ? `, ${direction === "asc" ? "ascending" : "descending"}` : ""}`} onPress={() => changeSort(key)} style={{ width, height: tableHeaderHeight, paddingHorizontal: 10, paddingVertical: 12, justifyContent: "center" }}>
      <Text style={{ ...text, fontWeight: "600", color: sort === key ? theme.colors.accent : theme.colors.foreground }}>{title}{sort === key ? direction === "desc" ? " ↓" : " ↑" : ""}</Text>
    </Pressable>;
  }
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: theme.colors.surface0 }} contentContainerStyle={{ padding: layout.compact ? 12 : 24, gap: 18 }}>
    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        ...
        <Pressable accessibilityRole="button" disabled={!rows.length} accessibilityState={{ disabled: !rows.length }} onPress={async () => { try { const csv = toCsv(sorted); if (!downloadCsv(csv)) await Share.share({ title: "Session usage", message: csv }); } catch { setError("Could not export session usage."); } }} style={buttonStyle}><Text style={text}>Export CSV</Text></Pressable>
      </View>
    </View>
    ...
    <Charts rows={rows} theme={theme} compact={layout.compact} />
    <View style={{ gap: 10 }}>
      <View style={{ flexDirection: "row", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        <Text accessibilityRole="header" style={{ ...text, fontSize: 17, fontWeight: "600" }}>Sessions</Text>
      ...
      <Text style={muted}>Press a column to sort, or a session name for details. Scroll sideways for more metrics. “—” means unknown; available values can be partial.</Text>
      <View style={{ flexDirection: "row", borderWidth: 1, borderColor: theme.colors.border, borderRadius: 10, overflow: "hidden" }}>
        <View style={{ width: firstColumnWidth, flexShrink: 0, borderRightWidth: 1, borderRightColor: theme.colors.border }}>
          <View style={{ backgroundColor: theme.colors.surface2 }}>{header("title", "Session", firstColumnWidth)}</View>
          {visible.map((row, i) => <Pressable key={row.session.id} accessibilityRole="button" accessibilityLabel={`Details for ${row.session.title}`} onPress={() => setSelected(row.session.id)} style={{ height: tableRowHeight, padding: 10, gap: 5, justifyContent: "center", backgroundColor: i % 2 ? theme.colors.surface1 : theme.colors.surface0, borderTopWidth: 1, borderTopColor: theme.colors.border }}>
        ...
        <ScrollView horizontal testID="session-usage-metrics" style={{ flex: 1, minWidth: 0 }}>
          <View>
            <View style={{ flexDirection: "row", backgroundColor: theme.colors.surface2 }}>
              {header("provider", "Provider", 100)}{header("project", "Project", 160)}{header("model", "Models", 180)}
              {columns.map((key) => header(key, METRICS[key].label, 142))}
              {header("endedAt", "Last activity", 170)}{header("startedAt", "Started", 170)}
            </View>
            {visible.map((row, i) => <View key={row.session.id} style={{ height: tableRowHeight, flexDirection: "row", backgroundColor: i % 2 ? theme.colors.surface1 : theme.colors.surface0, borderTopWidth: 1, borderTopColor: theme.colors.border }}>
              ...
              {columns.map((key) => <View key={key} style={{ width: 142, padding: 10, justifyContent: "center" }}><Text style={[text, { fontVariant: ["tabular-nums"], textAlign: "right" }]}>{formatMetric(key, metricValue(row, key))}</Text></View>)}
      ...
      {!visible.length && !query.isPending ? <Text style={muted}>{sessions.length ? "No sessions match these filters." : "No Claude or Codex sessions found on this host."}</Text> : null}
    ...
    <Modal title={selectedRow?.session.title ?? "Session details"} open={Boolean(selectedRow)} onOpenChange={(open) => { if (!open) setSelected(null); }}>
      <Modal.Content>{selectedRow ? <SessionDetails row={selectedRow} theme={theme} navigation={navigation} /> : <View />}</Modal.Content>
    </Modal>
