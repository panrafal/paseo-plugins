...
/**
 * Runs the system `grep` over the transcript files of the requested agents and turns the hits
 * into per-agent snippets. One grep invocation covers every file (chunked only if the argument
 * list would grow huge); its stdout is parsed as a stream because a single transcript line can
 * be several megabytes and a common word produces tens of megabytes of hits.
 */
