...
export type WorkspaceRecord = z.output<typeof WorkspaceRecordSchema>;
...
export interface CensusRecords {
  workspaces: WorkspaceRecord[];
  projects: ProjectRecord[];
  agents: AgentRecord[];
}

/** The raw daemon records the census and the search index are both built from. */
export async function loadCensusRecords(): Promise<CensusRecords> {
  const [workspaces, projects, agents] = await Promise.all([
    readWorkspaces(),
    readProjects(),
    loadAgentRecords(),
  ]);
  return { workspaces, projects, agents };
}

export interface WorkspaceLookup {
  byId: Map<string, WorkspaceRecord>;
  byCwd: Map<string, WorkspaceRecord>;
}

export function buildWorkspaceLookup(workspaces: readonly WorkspaceRecord[]): WorkspaceLookup {
...
/** The workspace an agent belongs to: by id when the record still exists, else by directory. */
export function resolveWorkspace(record: AgentRecord, lookup: WorkspaceLookup): WorkspaceRecord | null {
  if (record.workspaceId) {
    const byId = lookup.byId.get(record.workspaceId);
    if (byId) return byId;
  }
  return lookup.byCwd.get(normalizeCwd(record.cwd)) ?? null;
}

export async function buildCensus(): Promise<RpcOutput<typeof listHistory>> {
  const { workspaces: workspaceRecords, projects: projectRecords, agents: agentRecords } =
    await loadCensusRecords();
  const lookup = buildWorkspaceLookup(workspaceRecords);

  const agents = await mapWithConcurrency(agentRecords, RESOLVE_CONCURRENCY, async (record) => {
    const workspaceId = resolveWorkspace(record, lookup)?.workspaceId ?? null;
