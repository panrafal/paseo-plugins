...
export interface HistoryFilters {
  ...
  period: PeriodMode;
  /** Rank conversations by relevance through the index instead of listing grep hits. */
  ranked: boolean;
  regex: boolean;
...
/** Regex always greps; ranked mode needs the toggle on. */
export function searchMode(filters: HistoryFilters): SearchMode {
  return filters.regex || !filters.ranked ? "regex" : "ranked";
}
...
function rowScore(
  row: HistoryWorkspace,
  filters: HistoryFilters,
  matches: ReadonlyMap<string, AgentMatch>,
): number | null {
  let best: number | null = null;
  for (const agent of visibleAgents(row, filters)) {
    const score = matches.get(agent.id)?.score;
    if (score !== undefined && (best === null || score > best)) best = score;
  }
  return best;
}

/** Rows with a ranked hit first, best score first; the rest keep their activity order. */
export function rankWorkspaces(
  rows: readonly HistoryWorkspace[],
  filters: HistoryFilters,
  matches: ReadonlyMap<string, AgentMatch>,
): HistoryWorkspace[] {
  const scored = rows.map((row, index) => ({ row, index, score: rowScore(row, filters, matches) }));
  scored.sort((a, b) => {
...
/** Agents with a ranked hit first, best score first; the rest by activity. */
export function orderAgents(
  agents: readonly AgentSummary[],
  matches: ReadonlyMap<string, AgentMatch> | null,
): AgentSummary[] {
  if (!matches) return [...agents];
  return [...agents].sort((a, b) => {
    const left = matches.get(a.id)?.score;
    const right = matches.get(b.id)?.score;
    if (left !== undefined && right !== undefined) return right - left || compareAgents(a, b);
