 ...
 * search call runs `grep` over the agents' provider transcripts and returns a few matching
 * snippets per agent. Nothing here can carry an agent's persistence metadata.
 */
...
export const SearchErrorSchema = z.object({
  code: z.enum(["invalid-regex", "timeout", "superseded", "busy", "grep-failed"]),
  message: z.string(),
});
